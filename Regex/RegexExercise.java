...
public class RegexExercise {
    ...
    public static void main(String[] args) {
        ...
        RegexExercise template = new RegexExercise();
         ...
         */
        String[] patterns = {"\\."};
        String path = "C:\\Users\\L408-S1\\Documents\\NetBeansProjects\\Yezzer\\RegexExercise\\src\\RegexExercise\\twitter_data.txt";
        //String[] tweets = template.getTweets();
        String cleanedTweet = "";
//        for (int counter = 0; counter < template.fileReading(path).length; counter++) {
//            cleanedTweet = template.getCleanedTweet(template.fileReading(path)[counter], patterns);
//            System.out.println("Line " + (counter + 1) + ": " + cleanedTweet);
//        }

        System.out.println("OLD TWEETS:");
        for (int counter = 0; counter < template.getTweets().length; counter++) {
            String oldTweet = template.getTweets()[counter];
            System.out.println("Line " + (counter + 1) + ": " + oldTweet);
        }

        System.out.println("");
        System.out.println("CLEANED TWEETS:");
        for (int counter = 0; counter < template.getTweets().length; counter++) {
            cleanedTweet = template.getCleanedTweet(template.getTweets()[counter], patterns);
            System.out.println("Line " + (counter + 1) + ": " + cleanedTweet);
        }
        
        // Dapat cleaned tweets
        int wordsCount = template.countWords(template.getTweets());
        System.out.println("Number of Words:" + wordsCount);
        
        // Dapat cleaned tweets
        int specificWordsCount = template.countSpecificWord("what",template.getTweets());
        System.out.println("Number of Specific Words:" + specificWordsCount);
        
        
        int countCharacters = template.countCharacters(template.getTweets());
        System.out.println("Number of Specific Words:" + countCharacters);
    }
